...
- When updating high-level composition or JSON tree rules, update them in `scripts/build-json-rules.mjs` or the source fragments in `src/knowledge/fragments/`, which generate both `src/knowledge/json-rules.ts` (for Redactd JSON tree generation / MCP) and `skills/muibook-json-rules/SKILL.md` (for the Markdown skill).
